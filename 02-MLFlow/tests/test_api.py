...
Kör:  uv run pytest -v
Kräver att 'uv run train' körts så att registret innehåller en modell.

Vad är rimligt att testa i ett ML-projekt?
- Att tjänsten startar och modellen laddas från registret.
...
from iris_mlflow.api import app
...
def test_predict_returns_valid_contract(client):
    ...
    assert r.status_code == 200
    body = r.json()
    assert set(body) == {"species", "class_index", "probabilities", "model_version"}
